...
public abstract class Werknemer implements Comparable<Werknemer>, Serializable {
     ...
     */
    public Werknemer(int personeelsnr, int dag, int maand, int jaar, String naam) throws NaamException, PersoneelsNrException, DatumOutOfBoundsException, DatumIndiensttredingException
    {
        if (personeelsnr > 0) {
            this.personeelsnr = personeelsnr;
        }
        else {
            throw new PersoneelsNrException();
        }
     ...
     */
    public Werknemer(int personeelsnr, int dag, int maand, int jaar, String naam, Geslacht geslacht) throws NaamException, PersoneelsNrException, DatumOutOfBoundsException, DatumIndiensttredingException
    {
    ...
    public void setDatumInDienst(int dag, int maand, int jaar) throws DatumOutOfBoundsException, DatumIndiensttredingException
    {
        Datum tmp = new Datum(dag, maand, jaar);
        if (tmp.compareTo(WerknemersDatum.OPRICHTINGSDATUM) >= 0) {
            this.datumInDienst = new WerknemersDatum(dag, maand, jaar);
        }
        else {
            throw new DatumIndiensttredingException();
        }
    }
